...
const inputClass = 'h-[3.5rem] rounded-[1.1rem] border border-[#d8dfd2] bg-[#fbfcf8] px-4 text-base font-semibold text-[#102113] outline-none transition placeholder:text-[#6f7b6d] focus:border-[var(--slot4-accent-fill)] focus:bg-white'
const buttonClass = 'inline-flex h-[3.5rem] items-center justify-center rounded-full bg-[linear-gradient(135deg,var(--slot4-accent-fill),#3c68ff)] px-6 text-xs font-black uppercase tracking-[0.2em] text-white transition hover:-translate-y-0.5 disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1rem] border px-4 py-3 text-sm font-bold ${status === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-red-200 bg-red-50 text-red-700'}`}>{message}</p> : null}
